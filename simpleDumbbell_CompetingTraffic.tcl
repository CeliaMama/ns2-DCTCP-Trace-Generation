...
set flowClassifyTime 0.4

set sourceAlg DC-TCP-Sack
set sourceAlgMain DC-TCP-Newreno
#set sourceAlgMain DC-TCP-Cubic
set switchAlg RED
set lineRate 1.1Gb
...
if {$enableNAM != 0} {
    set namfile [open outComTrafficCubicVSNewRenoOneShort.nam w]
    $ns namtrace-all $namfile
}

set mytracefile [open mytracefileComTrafficCubicVSNewRenoOneShort.tr w]
set throughputfile [open thrfileComTrafficCubicVSNewRenoOneShort.tr w]
#Open the Trace file
set tf [open outComTrafficCubicVSNewRenoOneShort.tr w]
$ns trace-all $tf
...
proc finish {} {
        ...
        if {$enableNAM != 0} {
	    close $namfile
	    exec nam outComTrafficCubicVSNewRenoOneShort.nam &
	}
	exit 0
}
...
####Node 0 to be main traffic, node 1 to 7 competing traffic

for {set i 0} {$i < $N} {incr i} {
    if {$i < $M && [string compare $sourceAlgMain "DC-TCP-Newreno"] == 0} {
    	set tcp($i) [new Agent/TCP/Newreno]
    	set sink($i) [new Agent/TCPSink/DelAck]
    }
    if {$i < $M && [string compare $sourceAlgMain "DC-TCP-Sack"] == 0} { 
        set tcp($i) [new Agent/TCP/FullTcp/Sack]
    	set sink($i) [new Agent/TCP/FullTcp/Sack]
    	$sink($i) listen
    }
    if {$i < $M && [string compare $sourceAlgMain "DC-TCP-Cubic"] == 0} {
        set tcp($i) [new Agent/TCP/Linux]
        $tcp($i) set timestamps_ true
        $ns at 0 "$tcp($i) select_ca cubic"
        set sink($i) [new Agent/TCPSink/Sack1]
    }
    if {$i >= $M && [string compare $sourceAlg "DC-TCP-Newreno"] == 0} {
        set tcp($i) [new Agent/TCP/Newreno]
        set sink($i) [new Agent/TCPSink]
    }
    if {$i >= $M && [string compare $sourceAlg "DC-TCP-Sack"] == 0} { 
        set tcp($i) [new Agent/TCP/FullTcp/Sack]
        set sink($i) [new Agent/TCP/FullTcp/Sack]
        #set sink($i) [new Agent/TCPSink/DelAck]
        $sink($i) listen
    }
    if {$i >= $M && [string compare $sourceAlg "DC-TCP-Cubic"] == 0} {
        set tcp($i) [new Agent/TCP/Linux]
        $tcp($i) set timestamps_ true
